...
def plagiarism(name):
    ...
    if request.method == 'POST':
        if request.form['text'] != '' and request.files['file'].filename == '':
            ...
            if file and allowed_file(file.filename):
                ...
                # pdfFileObj = open('C:/Users/Milamathi/Downloads/plagiarism-check-main/plagiarism-check-main/uploads/{}'.format(filename), 'rb')
        ...
        if filename == "word":
            ...
            for i in range(len(inputan)):
                ...
                for j in range(len(list(search(query, tld=domain, num=10, stop=10, pause=2)))):
                    ...
                    hasil_link.append(
                        list(search(query, tld=domain, num=10, stop=10, pause=2))[j])
    ...
    # else:
        ...
        #         link_output.append(hasil_link_final[i])
            ...
            text = docx2txt.process('uploads/{}'.format(name))
            print(text)
            inputan += text.replace("\n", " ").split(". ")
            for i in range(len(inputan)):
                query = '"' + \
